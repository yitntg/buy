...
import { SortParams, FilterParams, CommentQueryParams } from '../domain/CommentRepository';
...
export const CommentList: React.FC<CommentListProps> = ({
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      if (result.items && result.items.length > 0) {
        await Promise.all([
          ...result.items.map((commentItem: Comment) => loadCommentLikes(commentItem.id)),
          ...result.items.map((commentItem: Comment) => loadReplyCount(commentItem.id))
        ]);
      }
  ...
  }, [productId, getCommentsUseCase, pagination.current, pagination.pageSize, sort]);
  ...
  const loadReplies = async (commentId: string) => {
    try {
      const result = await commentRepository.findByParentId(commentId, {
        page: 1,
        pageSize: 10,
        sortBy: 'createdAt',
        sortOrder: 'asc'
          ...
          <div key={comment.id} className="bg-white rounded-lg shadow-md p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center space-x-4">
                {/* 点赞按钮 */}
                <LikeButton
                  liked={user ? commentLikes[comment.id]?.includes(user.id) : false}
            ...
            <p className="text-gray-700 mb-2">{comment.getContent()}</p>
            {comment.getImages() && comment.getImages().length > 0 && (
              <div className="flex space-x-2 mt-2">
                {comment.getImages().map((image: string, index: number) => (
                  <img
                    key={index}
                    src={image}
                    alt={`评论图片 ${index + 1}`}
