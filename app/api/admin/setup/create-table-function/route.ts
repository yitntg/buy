...
export async function POST(request: NextRequest) {
  try {
    console.log('开始创建create_table函数流程')
    ...
    AS $$
    BEGIN
      EXECUTE 'CREATE TABLE IF NOT EXISTS ' || quote_ident(table_name) || ' (' || definition || ')';
    END;
    $$;

    -- 授予执行权限
    GRANT EXECUTE ON FUNCTION create_table TO authenticated;
    `
    ...
    try {
      ...
      return NextResponse.json({ 
        error: `创建create_table函数失败: ${error.message}` 
      }, { status: 500 })
    }
  } catch (error: any) {
    console.error('创建create_table函数过程中发生错误:', error)
    return NextResponse.json({ 
      error: `创建create_table函数失败: ${error.message}` 
    }, { status: 500 })
  }
