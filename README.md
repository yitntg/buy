...
/app                    # Next.js 应用目录
  ...
  /products             # 商品列表页面
  /api                  # API路由
  /context              # 上下文提供程序（如AuthContext）
  layout.tsx            # 根布局组件
  page.tsx              # 首页
/public                 # 静态资源
  /images               # 图片资源
/utils                  # 工具函数和服务
  /supabase             # Supabase相关功能（客户端、服务端、认证服务）
  upload.ts             # 文件上传工具
  payment.ts            # 支付网关集成
  timeout.ts            # API超时和重试逻辑
  index.ts              # 工具函数统一入口
/types                  # 类型定义
  supabase.ts           # Supabase数据库类型
  auth.ts               # 认证相关类型
/scripts                # 脚本文件目录
  fix-user-record.js    # 用户数据修复脚本
  check-auth.js         # 身份验证检查脚本
  verify-db.js          # 数据库验证脚本
  fix-revalidate.js     # 重新验证修复脚本
```

### 新版项目结构

新版项目采用按业务领域和技术层分离的目录结构，便于代码管理和未来可能的拆分。
...
本项目使用 Supabase (PostgreSQL) 作为数据库。数据库结构和变更应在Supabase控制台中管理，而不是通过应用代码。
...
这些环境变量需要设置在 `.env.local` 文件中（开发环境）或部署平台的环境变量配置中（生产环境）。
